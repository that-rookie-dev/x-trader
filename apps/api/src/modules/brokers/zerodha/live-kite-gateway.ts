...
export class LiveKiteGateway implements KiteGateway {
  ...
  private client(accessToken?: string) {
    const kc = new KiteConnect({ api_key: this.env.KITE_API_KEY ?? "" });
  ...
  async generateSession(requestToken: string): Promise<KiteSession> {
    const session = await this.client().generateSession(requestToken, this.env.KITE_API_SECRET ?? "");
    return session as KiteSession;
  }

  async invalidateAccessToken(accessToken: string): Promise<void> {
    await this.client(accessToken).invalidateAccessToken(accessToken);
  }

  async getProfile(accessToken: string): Promise<KiteProfile> {
    return (await this.client(accessToken).getProfile()) as KiteProfile;
  }

  async getMargins(accessToken: string) {
    return (await this.client(accessToken).getMargins()) as {
  ...
  async getHoldings(accessToken: string): Promise<unknown[]> {
    return (await this.client(accessToken).getHoldings()) as unknown[];
  }

  async getPositions(accessToken: string) {
    return (await this.client(accessToken).getPositions()) as { net?: unknown[]; day?: unknown[] };
  }
  ...
  async getQuote(accessToken: string, instruments: string[]) {
    return (await this.client(accessToken).getQuote(instruments)) as Record<string, never>;
  }

  async getLTP(accessToken: string, instruments: string[]) {
    return (await this.client(accessToken).getLTP(instruments)) as Record<string, { last_price?: number }>;
  }

  async getHistoricalData(accessToken: string, instrumentToken: string, interval: string, from: Date, to: Date) {
    const rows = (await this.client(accessToken).getHistoricalData(
  ...
  async placeOrder(accessToken: string, variety: string, params: Record<string, unknown>) {
    const result = (await this.client(accessToken).placeOrder(variety as never, params as never)) as { order_id: string };
    return result;
  }

  async modifyOrder(accessToken: string, variety: string, orderId: string, params: Record<string, unknown>) {
    return (await this.client(accessToken).modifyOrder(variety as never, orderId, params as never)) as { order_id: string };
  }

  async cancelOrder(accessToken: string, variety: string, orderId: string) {
    return (await this.client(accessToken).cancelOrder(variety as never, orderId)) as { order_id: string };
  }

  async getOrders(accessToken: string): Promise<unknown[]> {
    return (await this.client(accessToken).getOrders()) as unknown[];
  }
}
